...
pub fn mprotect(memory: []align(heap.page_alignment) u8, protection: u32) MProtectError!void {
    if (native_os == .windows) {
...
pub fn mmap(
    ptr: ?[*]align(heap.page_alignment) u8,
    length: usize,
    prot: u32,
    flags: system.MAP,
    fd: fd_t,
    offset: u64,
) MMapError![]align(heap.page_alignment) u8 {
    ...
    const err: E = if (builtin.link_libc) blk: {
        if (rc != std.c.MAP_FAILED) return @as([*]align(heap.page_alignment) u8, @ptrCast(@alignCast(rc)))[0..length];
        break :blk @enumFromInt(system._errno().*);
    } else blk: {
        const err = errno(rc);
        if (err == .SUCCESS) return @as([*]align(heap.page_alignment) u8, @ptrFromInt(rc))[0..length];
        break :blk err;
    };
    switch (err) {
        .SUCCESS => unreachable,
...
pub fn munmap(memory: []align(heap.page_alignment) const u8) void {
...
pub fn msync(memory: []align(heap.page_alignment) u8, flags: i32) MSyncError!void {
...
pub fn mincore(ptr: [*]align(heap.page_alignment) u8, length: usize, vec: [*]u8) MincoreError!void {
...
pub fn madvise(ptr: [*]align(heap.page_alignment) u8, length: usize, advice: u32) MadviseError!void {
